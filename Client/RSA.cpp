...
std::string RSA::Encrypt(std::string plain_text)
{
    ...
    FILE *f = fopen("../Client/RSA_Public_Key.txt", "r");
    if (f == NULL)
    {
...
std::string RSA::Decrypt(std::string secret_text)
{
    ...
    FILE *f = fopen("../Client/RSA_Secret_Key.txt", "r");
...
int2048_t RSA::HexToInt(std::string hex_str)
{
    ...
    for (int i = 0; i < hex_str.length(); i++)
    {
        ...
        if (hex_str[i] <= '9' && hex_str[i] >= '0')
            ...
            base = hex_str[i] - 'a' + 10;
        ret = ret + int2048_t(base) * FastExpMod(int2048_t(16), exp, -1);
    }
...
//     string res = rsa.Encrypt("0eaTDtWIV1I2Ct9a");
